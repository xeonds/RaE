...
(* ---------- shared schema defs (Q4: replaced by env passing, kept for construct) ---------- *)
let construct_defs : Ast.def list ref = ref []
let set_construct_defs defs = construct_defs := defs

(* ---------- file I/O ---------- *)
let read_file filename =
  let ic = open_in filename in let content = really_input_string ic (in_channel_length ic) in close_in ic; content
...
let write_uint value size =
  ...
  for i = 0 to size - 1 do Bytes.set buf i (Char.chr (Int64.to_int (Int64.logand !v 0xFFL))); v := Int64.shift_right_logical !v 8 done; buf

let reverse_bytes b =
  let len = Bytes.length b in let r = Bytes.make len '\000' in
  for i = 0 to len - 1 do Bytes.set r i (Bytes.get b (len - 1 - i)) done; r

let size_of_type = function I8|U8 -> 1 | I16|U16 -> 2 | I32|U32|F32 -> 4 | I64|U64|F64 -> 8 | BytesType(Some(IntLit(n,_))) -> n | _ -> 0

let crc32 bytes =
  let t = Array.make 256 0l in
  for i = 0 to 255 do let c = ref (Int32.of_int i) in
    for _j = 0 to 7 do c := if Int32.logand !c 1l <> 0l then Int32.logxor (Int32.shift_right_logical !c 1) 0xEDB88320l else Int32.shift_right_logical !c 1 done;
    t.(i) <- !c done;
  let crc = ref 0xFFFFFFFFl in
  for i = 0 to Bytes.length bytes - 1 do
    let idx = Int32.to_int (Int32.logxor !crc (Int32.of_int (Char.code (Bytes.get bytes i)))) land 0xFF in
    crc := Int32.logxor (Int32.shift_right_logical !crc 8) t.(idx) done;
...
let rec set_path path_expr env current rhs = match path_expr with
  | Ident ("_", _) -> rhs
  | Ident (name, _) -> (match current with VObj fields -> VObj ((name, rhs) :: List.remove_assoc name fields) | _ -> current)
  | FieldAccess (base, field, _) ->
    let base_val = eval_expr base env (ref current) in
    let new_base_val = match base_val with VObj fields -> VObj ((field, rhs) :: List.remove_assoc field fields) | _ -> base_val in
    set_path base env current new_base_val
  | ArrayAccess (base, idx, _) ->
    let i = eval_expr idx env (ref current) in
    let base_val = eval_expr base env (ref current) in
    let new_base_val = match base_val, i with VArray items, VInt n -> VArray (List.mapi (fun j v -> if j = n then rhs else v) items) | _ -> base_val in
    set_path base env current new_base_val
  | _ -> current

and eval_expr expr env current = match expr with
  | IntLit (n, _) -> VInt n | FloatLit (f, _) -> VFloat f | StringLit (s, _) -> VString s
  | Ident (id, _) -> (match id with
     | "_" -> !current
     | _ -> (match !current with VObj fields -> (try List.assoc id fields with Not_found -> try List.assoc id env with Not_found -> VNull) | _ -> (try List.assoc id env with Not_found -> VNull)))
  | BinaryOp (op, e1, e2, _) -> eval_binary_op op (eval_expr e1 env current) (eval_expr e2 env current)
  | FieldAccess (e, f, _) -> (match eval_expr e env current with
     | VObj fields -> (try List.assoc f fields with Not_found -> VNull)
     | VArray items -> VArray (List.map (fun item -> match item with VObj fields -> (try List.assoc f fields with Not_found -> VNull) | _ -> VNull) items) | _ -> VNull)
  | ArrayAccess (e, idx, _) -> (match eval_expr e env current, eval_expr idx env current with VArray items, VInt n -> (try List.nth items n with Failure _ -> VNull) | _ -> VNull)
  ...
  | BlockLit (lets, body, _) ->
    ...
    (match List.rev body with [] -> VNull | last :: rev_rest ->
       List.iter (fun e -> ignore (eval_expr e env' current)) rev_rest; eval_expr last env' current)
  ...
  | UnaryOp (op, e, _) ->
    ...
    (match op, v with Neg, VInt n -> VInt (-n) | Neg, VInt32 n -> VInt32 (Int32.neg n) | Neg, VInt64 n -> VInt64 (Int64.neg n) | Neg, VFloat f -> VFloat (-. f)
     | Not, VInt n -> VInt (if n = 0 then 1 else 0) | BitNot, VInt n -> VInt (lnot n land 0xFF) | BitNot, VInt32 n -> VInt32 (Int32.lognot n) | BitNot, VInt64 n -> VInt64 (Int64.lognot n) | _ -> VNull)

and eval_binary_op op v1 v2 =
  let vi n = VInt n and vi32 n = VInt32 n and vi64 n = VInt64 n in
  match op, v1, v2 with
  | Add, VInt a, VInt b -> vi (a+b) | Sub, VInt a, VInt b -> vi (a-b) | Mul, VInt a, VInt b -> vi (a*b) | Div, VInt a, VInt b -> vi (a/b)
  | Add, VInt32 a, VInt32 b -> vi32 (Int32.add a b) | Sub, VInt32 a, VInt32 b -> vi32 (Int32.sub a b) | Mul, VInt32 a, VInt32 b -> vi32 (Int32.mul a b) | Div, VInt32 a, VInt32 b -> vi32 (Int32.div a b)
  | Add, VInt64 a, VInt64 b -> vi64 (Int64.add a b) | Sub, VInt64 a, VInt64 b -> vi64 (Int64.sub a b) | Mul, VInt64 a, VInt64 b -> vi64 (Int64.mul a b) | Div, VInt64 a, VInt64 b -> vi64 (Int64.div a b)
  | Add, VInt32 a, VInt b -> vi32 (Int32.add a (Int32.of_int b)) | Add, VInt a, VInt32 b -> vi32 (Int32.add (Int32.of_int a) b)
  | Sub, VInt32 a, VInt b -> vi32 (Int32.sub a (Int32.of_int b)) | Mul, VInt32 a, VInt b -> vi32 (Int32.mul a (Int32.of_int b)) | Div, VInt32 a, VInt b -> vi32 (Int32.div a (Int32.of_int b))
  ...
  | Neq, VInt a, VInt b -> vi (if a<>b then 1 else 0) | Neq, VInt32 a, VInt32 b -> vi (if a<>b then 1 else 0) | Neq, VInt64 a, VInt64 b -> vi (if a<>b then 1 else 0)
  | Le, VInt a, VInt b -> vi (if a<=b then 1 else 0) | Le, VInt32 a, VInt32 b -> vi (if a<=b then 1 else 0)
  | Ge, VInt a, VInt b -> vi (if a>=b then 1 else 0) | Ge, VInt32 a, VInt32 b -> vi (if a>=b then 1 else 0)
  ...
  | RShift, VInt a, VInt b -> vi (a lsr b) | RShift, VInt32 a, VInt b -> vi32 (Int32.shift_right_logical a b) | RShift, VInt64 a, VInt b -> vi64 (Int64.shift_right_logical a b)
  | _ -> VNull
...
and eval_builtin name args env current = match name, args with
  ...
  | "select", [e] -> (match !current with VArray items -> VArray (List.filter (fun item -> match eval_expr e env (ref item) with VInt 0 -> false | _ -> true) items) | _ -> !current)
  | "echo", [e] -> let v = eval_expr e env current in
    (match v with VInt n -> Printf.printf "%d\n" n | VInt32 n -> Printf.printf "%ld\n" n | VInt64 n -> Printf.printf "%Ld\n" n | VFloat f -> Printf.printf "%f\n" f | VString s -> Printf.printf "%s\n" s | VBytes b -> Printf.printf "%s\n" (Bytes.to_string b) | VArray items -> Printf.printf "["; List.iteri (fun i item -> if i>0 then Printf.printf ", "; match item with VInt n -> Printf.printf "%d" n | VInt32 n -> Printf.printf "%ld" n | VString s -> Printf.printf "%s" s | _ -> Printf.printf "?") items; Printf.printf "]\n" | VObj fields -> Printf.printf "<obj %d>\n" (List.length fields) | VNull -> Printf.printf "null\n"); !current
  | "align", [e; n] -> (match eval_expr e env current, eval_expr n env current with VInt x, VInt y -> VInt ((x+y-1)/y*y) | _ -> VNull)
  | "bswap16", [e] -> (match eval_expr e env current with VInt n -> VInt (((n land 0xFF) lsl 8) lor ((n lsr 8) land 0xFF)) | _ -> VNull)
  | "bswap32", [e] -> (match eval_expr e env current with VInt32 n -> VInt32 (Int32.of_int (((Int32.to_int n) land 0xFF) lsl 24 lor (((Int32.to_int n) lsr 8) land 0xFF) lsl 16 lor (((Int32.to_int n) lsr 16) land 0xFF) lsl 8 lor ((Int32.to_int n) lsr 24))) | _ -> VNull)
  | "crc32", [e] -> let bytes = value_to_bytes (eval_expr e env current) in VInt32 (crc32 bytes)
  | "each", [Ident (varname, _); arr; blk] -> (match eval_expr arr env current with VArray items -> VArray (List.map (fun item -> eval_expr blk ((varname,item)::env) (ref item)) items) | _ -> VNull)
  | "checksum", [e] -> let bytes = value_to_bytes (eval_expr e env current) in let sum = ref 0 in for i = 0 to Bytes.length bytes - 1 do sum := !sum + Char.code (Bytes.get bytes i) done; VInt (!sum land 0xFFFF)
  | "checksum", [] -> let bytes = value_to_bytes !current in let sum = ref 0 in for i = 0 to Bytes.length bytes - 1 do sum := !sum + Char.code (Bytes.get bytes i) done; VInt (!sum land 0xFFFF)
  | "write", [e] -> let v = eval_expr e env current in let bytes = (match !current with VObj fields -> let file_name = (try match List.assoc "__file__" env with VString s -> s | _ -> "" with Not_found -> "") in if file_name <> "" then (match construct_binary file_name fields !construct_defs with VBytes b -> b | _ -> serialize_value !current env) else serialize_value !current env | _ -> serialize_value !current env) in (match v with VString filename -> let oc = open_out_bin filename in output oc bytes 0 (Bytes.length bytes); close_out oc; VInt 0 | _ -> VNull)
  | "object", _ -> VObj []
  | _ -> VNull
...
and construct_binary name field_vals struct_defs =
  ...
  let buf = Bytes.make !total '\000' in cur_off := 0;
  let checksum_fields = ref [] in
  List.iter (fun f ->
    let offset = match f.offset with Fixed n -> n | _ -> !cur_off in
  ...
  List.iter (fun (cs_offset, _f) -> let cs_val = crc32 (Bytes.sub buf 0 !cur_off) in let cs_bytes = write_uint (Int64.of_int32 cs_val) 4 in Bytes.blit cs_bytes 0 buf cs_offset (min 4 (Bytes.length buf - cs_offset))) !checksum_fields;
  VBytes buf

and eval_actions actions env current_val =
  let cur = ref current_val in
  match List.rev actions with [] -> VNull | last :: rev_rest ->
    List.iter (fun a -> ignore (eval_expr a env cur)) rev_rest; eval_expr last env cur

let values_equal a b = match a, b with
  | VInt x, VInt y -> x=y | VInt32 x, VInt32 y -> x=y | VInt64 x, VInt64 y -> x=y | VFloat x, VFloat y -> x=y | VString x, VString y -> x=y
  | VInt32 x, VInt y -> Int32.to_int x=y | VInt x, VInt32 y -> x=Int32.to_int y | VInt64 x, VInt y -> Int64.to_int x=y | VInt x, VInt64 y -> x=Int64.to_int y
  | _ -> false
...
let read_uint data offset size endian =
  ...
  for i = 0 to size - 1 do let pos = match endian with Some BE -> offset+size-1-i | _ -> offset+i in acc := Int64.logor !acc (Int64.shift_left (Int64.of_int (Char.code (Bytes.get data pos))) (8*i)) done; !acc

let read_sint data offset size endian =
  let raw = read_uint data offset size endian in let bits = size*8 in
  let sign_bit = Int64.shift_left Int64.one (bits-1) in if Int64.logand raw sign_bit <> Int64.zero then Int64.logor raw (Int64.lognot (Int64.sub sign_bit Int64.one)) else raw

let parse_value typ data endian = match typ with
  | I8 -> let v = Char.code (Bytes.get data 0) in VInt (if v land 0x80 <> 0 then v-256 else v) | U8 -> VInt (Char.code (Bytes.get data 0))
...
let parse_field_bytes typ endian data offset =
  ...
  if offset + size > Bytes.length data then raise (Engine_error (Printf.sprintf "Field read out of bounds: offset=%d size=%d len=%d" offset size (Bytes.length data)));
...
let parse_binary schema bytes =
  ...
  let rec parse_fields fields data base_offset prev_offset env field_ends = match fields with
    ...
    | f :: rest ->
      ...
      let val_, size = parse_field f data offset env (match f.attributes with
        | Ast.Endian (BE,_)::_ -> Some BE | _ -> None) in
  ...
  and parse_field f data offset env endian = match f.field_type with
    ...
    | Ast.ArrayType elem_type ->
      let count = ref 1 in
      List.iter (fun attr -> match attr with Ast.Count (expr,_) -> (match eval_expr expr env (ref VNull) with VInt n -> count := n | _ -> ()) | _ -> ()) f.attributes;
      let elems = ref [] in let off = ref offset in
      for _i = 1 to !count do
        let v, sz = match elem_type with
          | Ast.StructType name ->
            let info = lookup_struct name struct_registry in
            let env', fo, _ = parse_fields info.fields data !off !off [] [] in (VObj env', fo - !off)
