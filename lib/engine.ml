...
let parse_script_file filename =
  ...
  if String.length content >= 2 && String.sub content 0 2 = "#!" then
    ...
    (match lines with first :: rest when String.length first >= 2 && String.sub first 0 2 = "#!" -> String.concat "\n" rest | _ -> content)
  else content
...
let crc32 bytes =
  let t = Array.make 256 0l in for i = 0 to 255 do let c = ref (Int32.of_int i) in for _j = 0 to 7 do c := if Int32.logand !c 1l <> 0l then Int32.logxor (Int32.shift_right_logical !c 1) 0xEDB88320l else Int32.shift_right_logical !c 1 done; t.(i) <- !c done;
...
let rec set_path path_expr env current rhs = match path_expr with
  ...
  | ArrayAccess (base, idx, _) ->
    let i = eval_expr idx env (ref current) in let base_val = eval_expr base env (ref current) in
...
and eval_expr expr env current = match expr with
  ...
  | Ident (id, _) when id = current_id -> !current
  | Ident (id, _) -> (match !current with VObj fields -> (try List.assoc id fields with Not_found -> try List.assoc id env with Not_found -> VNull) | _ -> (try List.assoc id env with Not_found -> VNull))
...
and eval_binary_op op v1 v2 =
  let vi n = VInt n and vi32 n = VInt32 n and vi64 n = VInt64 n in match op, v1, v2 with
...
and eval_builtin name args env current = match name, args with
  ...
  | "echo", [e] -> let v = eval_expr e env current in Printf.printf "%s\n" (format_value v); !current
...
and eval_actions actions env current_val =
  let cur = ref current_val in match List.rev actions with [] -> VNull | last :: rev_rest ->
...
let read_sint data offset size endian = let raw = read_uint data offset size endian in let bits = size*8 in let sign_bit = Int64.shift_left Int64.one (bits-1) in if Int64.logand raw sign_bit <> Int64.zero then Int64.logor raw (Int64.lognot (Int64.sub sign_bit Int64.one)) else raw
